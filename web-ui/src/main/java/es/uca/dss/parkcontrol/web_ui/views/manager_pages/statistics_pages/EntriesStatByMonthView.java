...
@Route(value = "/manager/entries-stat-by-month")
@PageTitle("Entries Statistic By Month")
public class EntriesStatByMonthView extends VerticalLayout {
    ...
    public EntriesStatByMonthView() {
        addClassName("entries-stat-by-month-view");

        requestFactory.setHttpClient(httpClient);
        restTemplate.setRequestFactory(requestFactory);

        add(new H1("Entries Statistic By Month"));
        ...
        monthPicker.setLabel("Select a Month");
        add(monthPicker);
        ...
        showButton.addClickListener(e -> {
            ...
            if (selectedParking != null && selectedMonth != null) {
                System.out.println("http://localhost:8080/api/v1/managers/parking/" + selectedParking.getId() + "/statistics/entries?monthDate=" + selectedMonth.atDay(1).atStartOfDay().format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")));
                List<Record> records = Arrays.asList(restTemplate.getForObject("http://localhost:8080/api/v1/managers/parking/" + selectedParking.getId() + "/statistics/entries?monthDate=" + selectedMonth.atDay(1).atStartOfDay().format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")), Record[].class));
